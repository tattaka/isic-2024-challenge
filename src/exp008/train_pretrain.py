...
def load_image_ext(image_id: str, data_source: str) -> np.ndarray:
    return cv2.imread(f"../../input/{data_source}/train-image/image/{image_id}.jpg")[
        :, :, ::-1
    ]


class ISICExtDataset(Dataset):
    def __init__(self, meta_data: pd.DataFrame, mode="train", image_size=256):
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        data_source = self.meta_data.iloc[idx].data_source
        image = load_image_ext(image_id, data_source)
...
class ISICExtDataModule(LightningDataModule):
    def __init__(
        self,
...
def get_args() -> argparse.Namespace:
    ...
    parser = ISICExtDataModule.add_argparse_args(parser)
...
def main(args):
    ...
    isic2018_df = pd.read_csv(
        "../../input/isic-2018-jpg-256x256-resized/train-metadata.csv"
    )
    isic2018_df["data_source"] = "isic-2018-jpg-256x256-resized"
    isic2019_df = pd.read_csv(
        "../../input/isic-2019-jpg-256x256-resized/train-metadata.csv"
    )
    isic2019_df["data_source"] = "isic-2019-jpg-256x256-resized"
    isic2020_df = pd.read_csv(
        "../../input/isic-2020-jpg-256x256-resized/train-metadata.csv"
    )
    isic2020_df["data_source"] = "isic-2020-jpg-256x256-resized"
    external_train_meta_df = pd.concat(
        [isic2018_df, isic2019_df, isic2020_df]
    ).reset_index(drop=True)
    isfile_check = []
    for idx, row in external_train_meta_df.iterrows():
        isfile_check.append(
            os.path.isfile(
                f"../../input/{row.data_source}/train-image/image/{row.isic_id}.jpg"
            )
        )
    external_train_meta_df = external_train_meta_df[isfile_check].reset_index(drop=True)
    sgkf = StratifiedGroupKFold(n_splits=5, shuffle=True, random_state=42)
    external_train_meta_df["fold_id"] = -1

    for fold_id, (train_idx, val_idx) in enumerate(
        sgkf.split(
            external_train_meta_df,
            external_train_meta_df["target"],
            external_train_meta_df["patient_id"],
        )
    ):
        external_train_meta_df.loc[val_idx, "fold_id"] = fold_id
    ...
    for fold in range(5):
        ...
        external_train_meta_df = pd.concat(
            ...
            + [
                external_train_meta_df[
                    (external_train_meta_df.fold_id != fold)
                    & (external_train_meta_df.target == 1)
                ]
                for _ in range(args.pos_upsample_rate - 1)
            ]
        )
        datamodule = ISICExtDataModule(
            meta_data=external_train_meta_df,
            fold=fold,
            image_size=args.image_size,
            batch_size=args.batch_size,
            num_workers=args.num_workers,
        )
        model = ICIC2024LightningModel(
        ...
        auc_checkpoint = callbacks.ModelCheckpoint(
            filename="best_auc",
            monitor="val_auc",
            save_top_k=1,
            save_last=False,
